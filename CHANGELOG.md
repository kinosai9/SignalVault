...
- CLI: `signalvault diagnostics bundle --output <path> --limit-logs <n> --json`
...
- CLI: `signalvault diagnostics summary` (--json)
- CLI: `signalvault doctor` (alias for diagnostics summary)
...
P3 定位：把 signalvault 从"可运行的数据处理流水线"升级为"可恢复、可审计、可被 Agent 查询的投资知识库后端"。
...
### Done
- **mcp_server package** (`src/signalvault/mcp_server/`): 4 modules — `__init__.py`, `server.py`, `tools.py`, `serializers.py`
