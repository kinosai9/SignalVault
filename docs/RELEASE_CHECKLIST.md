...
- [x] `.gitignore` excludes: `.env`, `.env.local`, `*.key`, `*.pem`, `*.token`, `backup/`, `data/signalvault.db`, `data/user_settings.json`, `data/transcripts/`, `data/validation/`, `data/reports/*`, `logs/*`
...
- [ ] `python -m signalvault serve` starts without errors
...
- [ ] Real LLM analysis works: `python -m signalvault --youtube-url "URL" --no-mock`
...
- [ ] No `data/signalvault.db`, `data/user_settings.json`, `data/subtitle.srt` or other local-only files staged
