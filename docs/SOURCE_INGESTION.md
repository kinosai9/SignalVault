...
可阅读原文层的持久化规则见 `docs/SOURCE_PROVENANCE_PERSISTENCE_DESIGN.md`。后续视频逐字稿、网页原文稿、知识星球全文稿、PDF 页文本和上传文件正文应统一进入 Source Document / Source Segment 层，再被报告、观点、信号和搜索结果引用。
...
| 文本文件上传 | `/sources/files/import` | 上传 `.md` / `.txt` / `.html` / `.htm`，提取内容后归档 |
| PDF 工作流 | `pdf preview/extract/analyze` | 文本型 PDF 预览、提取、分析，保留页码证据 |
| ZSXQ 工作流 | `zsxq doctor/groups/import-topic/sync/analyze` | 只读导入已订阅知识星球主题，可进入分析 pipeline |

Web 统一入口：`GET /sources`。PDF/ZSXQ 当前以 CLI 能力为主，Web 承接纳入 SignalVault 前端体验改造。
...
- **Source Document / Source Segment** 是后续统一原文层，用于全文阅读、搜索片段定位和证据链追溯；它不替代 SourceArchive / Deep Notes / Report，而是为这些产物提供统一来源锚点。
...
- 可阅读原文层统一持久化（设计见 `docs/SOURCE_PROVENANCE_PERSISTENCE_DESIGN.md`）
- OCR 图片文字识别自动化
...
- Source Document / Source Segment schema 与迁移
- 视频完整带时间戳原始字幕与已翻译逐字稿持久化
- 网页原文稿、知识星球全文稿、PDF 页文本统一接入原文层
- Unified Knowledge Search 增加原文材料和原文片段结果类型
- PDF Web 上传/分析入口
- ZSXQ Web 管理入口
- RSS/Atom adapter
