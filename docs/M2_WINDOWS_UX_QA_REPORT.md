...
**2026-07-21 更新：M2-R 生命周期阻断修复和 Windows 聚焦复验均已完成，最终结论见第 16 节。**
...
**首次复验结论：否；此结论已被第 16 节最终封板结论取代。**当时的阻断项为：
...
| 阻断项 | 修复 | 验证 |
|---|---|---|
| P0-1: Windows 重复启动 | Win32 API 进程检测 + health-first 状态机 | 5 个 Windows probe tests 通过 |
| P0-2: 初始化异常边界 | 统一 try/except/finally 从最早可失败操作开始 | 4 个 init failure tests 通过 |
| P0-3: 服务提前退出 | keep-alive 后 shutdown_requested 判定 + queue.Queue 异常传递 | 3 个 early exit + 3 个 exception propagation tests 通过 |
| P1-4: Ctrl+C 验收 | 代码和自动化就绪 | 人工验收结果见第 16 节 |

### 变更文件

- `src/signalvault/launcher.py`：Win32 进程检测、health-first 状态机、PID 所有权、损坏 PID 归档、可行动错误文案和线程异常传递。
- `tests/test_launcher.py`：87 tests（+52）。
- `docs/M2_LAUNCHER_ACCEPTANCE_REPORT.md`：新增 M2-R 章节。
...
| `pytest --collect-only -q` | 2518 tests |
...
3 项非 UI 失败和 3 项 UI smoke 失败均为已记录的 C3、channels 或 FastAPI 兼容性存量，**M2-R 新增回归为 0**。

### Windows 实测

首次启动、重复启动、冲突、服务提前退出、Ctrl+C 和恢复的最终结果见第 16 节。

## 16. Codex 聚焦复验与 M2 正式封板（2026-07-21）

### 复验边界与隔离

- 平台：Windows；Python 3.12.9；基线提交 `c76fe12`。
- 仅复验首次启动、重复启动、已有实例提示、冲突错误、服务提前退出、浏览器失败降级、退出后恢复和 onboarding 浏览器旅程；未重跑 C2/C3 全视觉矩阵。
- 最终有效浏览器证据使用临时源码副本和全新 `SIGNALVAULT_HOME`，并清空 `DATA_DIR`、`LOG_DIR`、`DB_PATH` 旧覆盖；Mock AI、临时 Vault、空 SQLite，运行目录不含 `.env`。
- 首次浏览器尝试发现源码目录的兼容迁移会探测仓库 `data/user_settings.json`，该轮证据立即作废；未将其计入结论，也未修改 ConfigService、SecretStore、Vault 或底层进程状态机。

### 聚焦结果

| 项目 | 结果 | 文案与日志证据 |
|---|---|---|
| 首次启动 | 通过 | 前台依次显示“正在启动”“日志位置”“启动成功”“访问地址”；health 为 `ok/signalvault` |
| 重复启动 / 已有实例 | 通过 | 第二次指定 18109 仍复用 18101，返回 0，PID 内容不变；日志记录 `Existing healthy instance found`、`Reusing existing instance` |
| 冲突错误 | 通过 | 返回非零、不启动第二服务、不覆盖 PID；错误提示说明冲突原因、处理动作和日志位置 |
| 服务提前退出 | 通过 | health 前退出、health 后无信号退出均返回非零，不等待完整超时，并清理自有 PID |
| 浏览器失败降级 | 通过 | `open=False` 与抛异常均转换为手动 URL 提示；服务保持运行，已有实例不被清理 |
| 退出后恢复 | 通过 | 受控构造异常退出后 stale PID 被归档，新实例在 18101 恢复健康；SIGINT/SIGTERM 自动化用例通过 |
| onboarding 旅程 | 通过 | welcome → Mock → 跳过 Obsidian → complete → dashboard；CSS 941 条规则、横向溢出 0、无 API Key、无浏览器 error |
| 关闭浏览器 / 再次 launch | 通过 | 关闭标签页后 health 仍正常；再次 launch 复用相同 PID，重新访问直接进入 dashboard，不重开 onboarding |

Windows 重复启动与 Ctrl+C 的前台人工验收由人工验收结果确认通过；本轮没有用自动化替代或改写该人工结论。
...
| `ruff check src/ tests/` | **All checks passed** |
| health + onboarding + AppPaths + setup status | **90 passed，2 warnings** |
| Launcher 聚焦场景 | **24 passed，63 deselected** |
| M2-R 回归判断 | **无新增回归**；沿用 M2-R 全量基线，本轮未重跑 C2/C3 视觉矩阵 |
...
**M2 正式标记为通过，具备进入 M3-A Briefcase spike 的条件。**

以下项目仍未验证，不得表述为 macOS 实机通过：

- macOS Finder / Dock / Cmd+Q。
- `Application Support` 实际权限。
- macOS 默认浏览器。

这些项目不阻塞 M3-A spike，但会阻塞 M3 最终验收。
