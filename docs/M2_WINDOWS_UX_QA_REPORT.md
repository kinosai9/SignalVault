...
**2026-07-21 更新：M2-R 生命周期阻断修复已完成，见下方第十五节。进入 M3-A 前仍需 Windows 实测 + Ctrl+C 人工验收。**
...
| P1-4: Ctrl+C 验收 | 代码就绪；待人工前台终端验收 | — |

### 变更文件

- `src/signalvault/launcher.py` — 完整重写（~600 行）
  - Win32 `_win32_process_exists()` via ctypes
  - `DetectionOutcome` health-first 状态机
  - `_LaunchState` PID 所有权跟踪
  - `_archive_pid_file()` 损坏 PID 原子归档
  - `_error_*` 三条可行动错误文案
  - `_UvicornRunner` queue.Queue 异常传递
...
3 项非 UI 失败：2 项 C3 存量 + 1 项 FastAPI `_IncludedRouter` 兼容性（非 M2-R 引入）。
3 项 UI smoke 失败：2 项 channels 存量 + 1 项 C3 onboarding guard 重定向（非 M2-R 引入）。
...
修复后需执行：
1. 首次 launch → 8000 健康
2. 第二次 launch → 检测已有实例，浏览器打开，无第二实例，PID 不变
3. 构造冲突（PID 存活但 health 不可达）→ 返回非零，无第二实例
4. 服务提前退出 → 返回非零，PID 清理
5. Ctrl+C 人工验收 → 优雅退出，PID 清理，端口释放，可重新启动
