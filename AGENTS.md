...
**项目进入 Release Engineering 封板，前端体验已实现四条主用户动线。** P0–P7 已交付到多源摄入、PDF 分析、ZSXQ 只读导入、统一搜索、轻量图谱、SourceDocument/SourceSegment 原文层、诊断中心与诊断包导出。当前 `python -m pytest --collect-only -q` 可收集 2013 tests。

当前前端主线：依据 `docs/FRONTEND_EXPERIENCE_EXECUTION_PLAN.md`，在不改变后端数据契约的前提下，已将现有 Web Console 改造成 SignalVault 的四条主用户动线：变化雷达、信息源工作台、导入向导、知识库搜索。
...
```bash
python -m pytest tests/ -v    # 当前可收集 2013 tests，全部使用 mock provider
...
| `docs/USER_GUIDE.md` | 用户 | 首次使用、日常动线、来源导入、证据阅读与排障 |
| `docs/RELEASE_ENGINEERING_AUDIT.md` | 发布 | 封板事实盘点、验证证据、阻断项与发布判断 |
| `docs/RELEASE_CHECKLIST.md` | 发布 | Release Candidate 与正式发布门禁 |
