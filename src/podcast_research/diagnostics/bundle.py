...
class DiagnosticBundleBuilder:
    ...
    def _collect_search_graph(self) -> None:
        try:
            if self.session is None:
                from podcast_research.db.session import get_session as _gs
                s = _gs()
                _close = True
            else:
                s = self.session
    ...
    def _collect_readme(self) -> None:
        readme = (
            "Diagnostic Bundle\n"
            "=================\n\n"
            "This bundle was generated by podcast_research (P7-D).\n"
    ...
    def _get_version() -> str:
        try:
            from podcast_research import __version__
            return str(__version__)
        except (ImportError, AttributeError):
            pass
        try:
            import importlib.metadata
            return importlib.metadata.version("podcast_research")
        except Exception:
            return "unknown"
