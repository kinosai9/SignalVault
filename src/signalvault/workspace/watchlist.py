...
def resolve_watchlist_name(
    ...
    from signalvault.llm_wiki.taxonomy import TOPIC_CANONICAL_MAP
    if item_type == "topic":
...
def get_suggested_companies(snapshot) -> list[str]:
    ...
    """
    from signalvault.llm_wiki.context_builder import HIGH_VALUE_COMPANIES
    suggestions = []
...
def _build_item_brief(
    ...
    """
    from signalvault.workspace.canonical_view import build_watchlist_evidence
...
def write_watchlist_brief(vault_path: Path, brief_md: str) -> Path:
    ...
    from signalvault.workspace.managed_block import _upsert_managed_block
