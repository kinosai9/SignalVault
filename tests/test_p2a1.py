...
def test_investment_view_defaults_v2():
    """新字段有合理默认值，不影响旧调用。"""
    from signalvault.analysis.models import InvestmentView
    v = InvestmentView(
        target_name="NVIDIA",
...
def test_tech_industry_insight_serializable():
    from signalvault.analysis.models import TechIndustryInsight
...
def test_entity_type_enum_v2():
    from signalvault.analysis.models import Entity
...
def test_prompt_contains_investment_boundary_rules():
    from signalvault.llm.prompts import EXTRACT_FACTS_SYSTEM
...
def test_prompt_contains_report_structure():
    from signalvault.llm.prompts import RENDER_REPORT_SYSTEM
...
def test_prompt_contains_focus_areas_placeholder():
    from signalvault.llm.prompts import EXTRACT_FACTS_USER
...
def test_mock_report_contains_tech_insights_section():
    ...
    from signalvault.analysis.models import ExtractionResult, TechIndustryInsight
    from signalvault.llm.mock_provider import MockLLMProvider
